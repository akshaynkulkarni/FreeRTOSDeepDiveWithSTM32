...
void sometask(void *params) {
	...
	while (1) {
		...
		if(xTaskNotifyWait( 0, 0, NULL, LED_Delays[taskno])) { //Notification received
			...
			HAL_GPIO_WritePin(LED_PORTS[taskno], LED_PINS[taskno], GPIO_PIN_SET);
			snprintf(msg, 30,"Deleted task: %s\n", led_names[taskno]);
			SEGGER_SYSVIEW_PrintfTarget(msg);
			next_task_handle = (taskno == 2) ?
								NULL :	&led_task_handle[taskno + 1];
...
void handle_button_press() {
	...
	snprintf(msg, sizeof(msg),"Button pressed....\n");
	SEGGER_SYSVIEW_PrintfTarget(msg);
	...
	if(!next_task_handle) {
		...
		// refer: https://www.freertos.org/xTaskNotifyFromISR.html
		portYIELD_FROM_ISR( xHigherPriorityTaskWoken );

		if(*next_task_handle == led_task_handle[2]) {
			snprintf(msg, sizeof(msg),"Producer consumer task starting..\n");
			SEGGER_SYSVIEW_PrintfTarget(msg);

			//trigger consumer producer tasks

			xTaskNotifyFromISR(producer_handle, 0, eNoAction, NULL);
			xTaskNotifyFromISR(consumer_handle[0], 0, eNoAction, NULL);
			xTaskNotifyFromISR(consumer_handle[1], 0, eNoAction, NULL);
